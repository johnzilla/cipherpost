...
must_haves:
  truths:
    ...
    - "SPEC.md §8 has test-vector sub-sections 8.1 (OuterRecordSignable, 192-byte fixture) and 8.2 (ReceiptSignable, 424-byte fixture) with inline hex + reproduction stanza + Ed25519 signature over a deterministic seed ([0u8;32] as the primary path, or SHA-256-derived from 'cipherpost/v1/spec-test-vector' as documented fallback if ed25519-dalek =3.0.0-pre.5 rejects the all-zero seed at construction time) whose derivation is documented verbatim in SPEC §8"
