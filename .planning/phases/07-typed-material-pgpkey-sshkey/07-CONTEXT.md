...
- **D-P7-03 · PGP Ed25519-minimal round-trip MUST pass; SSH round-trip ships `#[ignore]`'d from day 1. [AMENDED post-research 2026-04-24]**
  - **PGP:** minimal transferable public key (Ed25519 primary + one self-cert + UID ≤20 chars — research showed 43-char UID in original CONTEXT.md drafts pushed total over budget). Research prediction: ~180-200 B raw → ~750-830 B encoded — fits under 1000 B. `pgp_self_round_trip_recovers_packet_stream` test MUST pass.
  - **SSH:** research empirically predicted minimum OpenSSH v1 Ed25519 blob (`ssh-keygen -t ed25519 -C "" -N ""`) produces ~321 B raw → ~1340 B encoded, 34% over the 1000 B BEP44 ceiling. The D-P7-03 original-draft fallback clause is active from day 1. `tests/ssh_roundtrip.rs` ships with the round-trip test `#[ignore]`'d with exact note: `wire-budget: minimum OpenSSH v1 Ed25519 blob exceeds 1000-byte BEP44 ceiling (~1340 B encoded) — see Pitfall #22 / v1.2 milestone`.
  - **Positive `WireBudgetExceeded` tests (D-P7-02) unchanged and ship for BOTH variants regardless.**
  - **Honest messaging in SPEC.md:** section on Pitfall #22 explicitly lists what-works-today matrix (PGP Ed25519-minimal OK; SSH does not round-trip at any size today).

### B. PGP ingest + preview (mirrors Phase 6 X.509 pattern)

- **D-P7-04 · PGP crate: `pgp = "0.19.0"` locked. [AMENDED post-research 2026-04-24]** `pgp = { version = "=0.19.0", default-features = false, features = ["alloc"] }`. Exact-pin (`=0.19.0`) because research findings (MSRV 1.88; RUSTSEC-2023-0071 via rsa 0.9; ed25519-dalek 2.1.1 unconditional pull) are all version-specific — any upstream change to any of these requires re-planning. MIT/Apache-2.0. Dep tree MUST NOT pull `ring` or `aws-lc` (verified by research). Dep tree WILL pull `ed25519-dalek 2.1.1` (unconditional) — this is the source of the coexistence with `=3.0.0-pre.5`; see D-P7-22. **Rejected:** `sequoia-openpgp` — heavy; default C bindings; rework CONTEXT.md. **Rejected:** hand-roll RFC 4880 — v4-vs-v5 fingerprint hashing, UID encoding, sig-subpacket semantics too much scope.
...
- **D-P7-10 · SSH crate: `ssh-key = "0.6.7"` with `default-features = false, features = ["alloc"]`. [VERIFIED post-research 2026-04-24]** Research confirmed source-level that the `ed25519` feature can be disabled while `PrivateKey::from_openssh()` + `Fingerprint::compute()` still work for Ed25519 keys — only the `ed25519-dalek` interop `TryFrom` impls are cfg-gated, and those are not needed for parse + fingerprint. Dep tree with this config: **CLEAN** (no `ed25519-dalek 2.x` from SSH side). Plan 05 still MUST run `cargo tree | grep ed25519-dalek` as a regression guard to catch upstream feature-shape changes, but the EXPECTED outcome is "no 2.x leak from ssh-key" — the 2.x that IS in the tree comes from `pgp 0.19.0` (D-P7-04), not SSH. **Original fallback to accept-coexistence is now moot for SSH.**
...
### E. Research-driven amendments [added 2026-04-24 post-research]

- **D-P7-20 · MSRV bump from 1.85 → 1.88 in Plan 01.** `pgp 0.19.0` requires Rust 1.88. Plan 01 task: update `Cargo.toml` `rust-version = "1.88"`; update `.github/workflows/*.yml` rust-toolchain pin if explicit; update `README.md` / `SPEC.md` "Requirements" section. Rust 1.88 has been stable since mid-2025 (~10 months at Phase 7 plan time) — low compat risk. Single breaking change for contributors compiling locally; documented in Plan 01 SUMMARY and CHANGELOG-style prose in SPEC.md §Requirements. **Rejected:** pin older `pgp` version for 1.85 compat — research noted pgp 0.18.x API drift and unknown CVE status; adds research work for marginal gain.

- **D-P7-21 · RUSTSEC-2023-0071 (Marvin Attack via `rsa 0.9`) accepted via `deny.toml` ignore + documented rationale.** Plan 01 adds `[advisories] ignore = ["RUSTSEC-2023-0071"]` to `deny.toml` with exact comment: `# Marvin Attack on rsa 0.9 requires RSA private-key decryption/signing operations. Cipherpost uses the pgp crate only for packet parsing and fingerprint extraction — we do not decrypt or sign with RSA. Advisory does not apply to our usage. Remove this ignore when upstream rsa crate patches.` SPEC.md adds a §Supply-Chain Deferrals subsection recording the same rationale. `cargo-audit` / `cargo-deny` CI must be green after the ignore lands. **Rejected:** switch PGP crates to avoid rsa 0.9 transitive — reopens D-P7-04; adds research work. **Rejected:** ship Phase 7 with CI red — violates the 'CI-enforced `cargo audit` / `cargo deny`' project rule in CLAUDE.md.

- **D-P7-22 · ed25519-dalek coexistence acceptance lives in Plan 01 (PGP), NOT Plan 05 (SSH). [CORRECTS pre-research assumption of SSH-10]** Research confirmed `pgp 0.19.0` unconditionally pulls `ed25519-dalek 2.1.1`. This is where the coexistence with `=3.0.0-pre.5` comes from, regardless of what SSH does. Plan 01 owns:
  - `cargo tree -p ed25519-dalek` evidence block in Plan 01 SUMMARY (expected output: both `2.1.1` and `=3.0.0-pre.5` present)
  - `SPEC.md` §Supply-Chain Deferrals entry documenting "ed25519-dalek 2.1.1 coexists with 3.0.0-pre.5 pin because `pgp 0.19.0` pulls 2.1.1 unconditionally. Binary carries two Ed25519 impls. Low runtime risk — each crate uses its own pinned version. Remove when either (a) pgp releases a version that drops ed25519-dalek 2.x, or (b) cipherpost migrates to pkarr-vers-that-supports-ed25519-dalek-stable-3.x."
  - Plan 01 dep-tree guard test asserts the SPECIFIC expected shape (both versions present, no unexpected third version).
  - SSH-10 requirement is still MET by this — "cargo tree | grep ed25519-dalek pre-flight result is documented in Phase 7 plan 01 — either 'no 2.x leak' or explicit coexistence acceptance recorded before any ssh-key code ships." The documented-coexistence outcome lands in Plan 01 (PGP); Plan 05 (SSH) adds its own `cargo tree -p ed25519-dalek` regression test to confirm SSH does NOT add a third version.

### Claude's Discretion

- ~~Exact `pgp` crate version~~ — **RESOLVED by research: `=0.19.0`. See D-P7-04.**
- ~~Exact `ssh-key` crate version~~ — **RESOLVED by research: `0.6.7`. See D-P7-10.**
- Fixture byte layout: hand-crafted single PGP packet stream via `pgp` crate round-trip (UID MUST be ≤20 chars per research wire-budget prediction — suggest `cp-fixture <f@cp.t>` at 18 chars); hand-crafted SSH key via `ssh-keygen -t ed25519 -C "" -N ""` invoked once, bytes committed. Reproduction recipes documented in sibling `.txt` files per Phase 6 pattern.
